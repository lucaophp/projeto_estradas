...
class TopoGrafia:
    ...
    def initGui(self):
        ...
        icon_transfer = ':/plugins/TopoGrafia/app/resources/icons/icontransfer.png'
        self.add_action(
            icon_path,
        ...
        self.add_action(
            icon_transfer,
            text=self.tr(u'Transferência de Pontos Cotados'),
            callback=self.run_transferencia,
            parent=self.iface.mainWindow()
        )
    ...
    def run_transferencia(self):
        if not self.conf.savefile() is None:
            self.conf.transferencia()
