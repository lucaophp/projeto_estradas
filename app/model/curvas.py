...
class Curvas:
    ...
    def list_curvas(self):
        ...
        curvas = con.execute(
            "SELECT CURVA.id,CURVA.tipo,CURVA.velocidade,CURVA.raio_utilizado,CURVA.emax,CURVA.estaca_inicial_id,CURVA.estaca_final_id FROM CURVA INNER JOIN TABLECURVA ON CURVA.TABLECURVA_id=TABLECURVA.id WHERE TABLECURVA.TABLEESTACA_id=%d  ORDER BY CURVA.estaca_inicial_id ASC" % self.id_filename).fetchall()
        con.close()
    ...
    '''def gera_estacas(self,dist):
        ...
        progressiva = 0.0
        for c in curvas:
            #pego os dados da curva

            detalhes = self.get_curva_details(id_curva=c[CURVA_ID])
            # epc - Ponto inicial da curva
            epc = float(detalhes[EPC])
            # epc - Ponto final da curva
            ept = float(detalhes[EPT])
            # epi - Novo ponto de vertice da curva
            epi = float(detalhes[EPI])
            ...
            #criar as estacas do ponto inicial ao ponto epc
            while progressiva<epc:
                estaca = []
